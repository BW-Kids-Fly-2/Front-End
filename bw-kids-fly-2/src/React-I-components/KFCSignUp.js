...
const SignUpForms = ({ values, touched, errors, status, ...props }) => {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    props.addUser(user).then(() => props.history.push('/assistant'));
    setUser({
      username: '',
      password: '',
    });
      ...
      <FormContainer>
        <div className="signup">
          <div className="userinfo">
            <h3> Welcome to the Kids Fly Connect Team {user.name} ! </h3>
          </div>
...
export default connect(null, { addAssistant })(FormikAssistSignUp);
